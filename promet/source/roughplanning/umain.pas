...
uses uBaseApplication, uData, uBaseDbInterface,uWikiFrame,
  ...
  uProjects,uPrometFrames,uBaseDbClasses,uroughpklanningframe;
procedure TfMain.DoCreate;
begin
  with Application as IBaseApplication do
    begin
      SetConfigName('RoughPlanning');
    end;
...
begin
  ...
  aDocuments.Destroy;
  //Projects
  uroughpklanningframe.AddToMainTree(acNewStatistic);
...
begin
  ...
  if copy(aLink,0,8) = 'CUSTOMER' then
    ...
    begin
      aFrame := TfRoughPlanningFrame.Create(Self);
      aFrame.OpenFromLink(aLink);
      pcPages.AddTab(aFrame);
      aFrame.SetLanguage;
      Result := True;
