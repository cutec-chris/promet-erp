...
uses
  ...
  imapsend, mimepart, uData, uBaseDBInterface, Utils, uMessages, uBaseDBClasses,
...
type
  TIMAPReceiver = class(TBaseCustomApplication)
  private
    TextThere : Boolean;
    HtmlThere : Boolean;
    aSender: String;
    ...
    begin  //Servertype;Server;Username;Password;Active
      if copy(mailaccounts,0,pos(';',mailaccounts)-1) = 'POP3' then
        begin
          ...
          imap.TargetHost := copy(mailaccounts,0,pos(';',mailaccounts)-1);
          if pos(':',imap.TargetHost) > 0 then
            begin
              imap.TargetPort := copy(imap.TargetHost,pos(':',imap.TargetHost)+1,length(imap.TargetHost));
              imap.TargetHost := copy(imap.TargetHost,0,pos(':',imap.TargetHost)-1);
            end;
          Info(Format(strReceivingMailsFrom,[copy(mailaccounts,0,pos(';',mailaccounts)-1)]));
          mailaccounts := copy(mailaccounts,pos(';',mailaccounts)+1,length(mailaccounts));
          imap.UserName := copy(mailaccounts,0,pos(';',mailaccounts)-1);
          mailaccounts := copy(mailaccounts,pos(';',mailaccounts)+1,length(mailaccounts));
          imap.Password := copy(mailaccounts,0,pos(';',mailaccounts)-1);
          mailaccounts := copy(mailaccounts,pos(';',mailaccounts)+1,length(mailaccounts));
          imap.AutoTLS:=True;
          imap.Timeout:=12000;
          imap.Sock.ConnectionTimeout:=12000;
          tmp := mailaccounts;
          ...
          omailaccounts := omailaccounts+'POP3;'+imap.TargetHost+';'+imap.UserName+';'+imap.Password+';'+copy(mailaccounts,0,pos(';',mailaccounts)-1)+';';
          if DoArchive then
          ...
          if copy(mailaccounts,0,pos(';',mailaccounts)-1) = 'YES' then
            if imap.Login then
              begin
                ...
                for i := start+1 to MList.Count-1 do
                  begin
                    try
                      ...
                      DoDownload := True;
                      imap.List(MID);
                      atmp := imap.ResultString;
                      if StrToIntDef(copy(atmp,rpos(' ',atmp)+1,length(atmp)),0) > 10*(1024*1024) then
                        begin
                          imap.Top(MID,0); //Header holen
                          msg := TMimeMess.Create;
                          msg.Lines.Text:=imap.FullResult.Text;
                          msg.DecodeMessage;
                      ...
                      if DoDownload then
                        begin
                          ...
                          and (not DeletedItems.DataSet.Locate('LINK','MESSAGEIDX@'+aMID,[])) then
                            begin
                              ureceivemessage.CheckHeader(aMid,msg,imap.UserName);
                              if imap.Retr(MID) then //Naricht holen
                                begin
                                  ReceiveMessage(aMID,imap.FullResult,Message);
                                  MessageHandler.SendCommand('prometerp','Message.refresh');
                                  if DoArchive and Assigned(imap.FullResult) then
                                    begin
                                      ArchiveMessage(aMID,imap.FullResult);
                                      if DoDelete then
                                        begin
                                          imap.Dele(MID);
                                        end;
               ...
               begin
                 if imap.ResultString = '' then
                   Error(strMailLoginFailed)
                 else
                   Error(imap.ResultString);
               end;
             imap.Free;
...
function TIMAPReceiver.GetSingleInstance: Boolean;
begin
  Result:=False;
end;

constructor TIMAPReceiver.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
end;
function TIMAPReceiver.CommandReceived(Sender: TObject; aCommand: string
  ): Boolean;
var
  aUser: String;
begin
...
var
  Application: TIMAPReceiver;
begin
  Application:=TIMAPReceiver.Create(nil);
  Application.Run;
  Application.Free;
end.
