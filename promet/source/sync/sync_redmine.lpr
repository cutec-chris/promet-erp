...
resourcestring
  strFailedToLoadConfig          = 'sync_redmine.conf does not exists !';
  strConnectFailed               = 'Cant connect to redmine DB';
...
begin
  ...
  if not FileExists('sync_redmine.conf') then
    raise Exception.Create(strFailedToLoadConfig);
  aConfig := TIniFile.Create('sync_redmine.conf');
  ...
  rmConn.Protocol:=aConfig.ReadString('database','protocol','postgresql-8');
  rmConn.HostName:=aConfig.ReadString('database','host','');
  rmConn.Database:=aConfig.ReadString('database','database','');
  rmConn.Port:=aConfig.ReadInteger('database','port',5432);
  ...
  rmQuerry.SQL.Text:='select * from "projects"';
  rmQuerry.Open;
