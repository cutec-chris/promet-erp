      ...
      if Assigned(ManagedFieldDefs) then
        with ManagedFieldDefs do
          begin
            ...
            Add('NAME',ftString,160,True);
            Add('EXTENSION',ftString,15,false);
            Add('DOCUMENT',ftBlob,0,False);
          end;
    end;
end;
...
function TDocuments.GetUsedFields: string;
var
  tmpFields : string = '';
  i: Integer;
  aOldLimit: Integer;
          ...
          with Self.DataSet as IBaseDBFilter,Self.DataSet as IBaseManageDB do
            begin
              ...
              for i := 0 to DataSet.FieldDefs.Count-1 do
                if  (DataSet.FieldDefs[i].Name <> 'DOCUMENT')
...
procedure TDocuments.DefineFields(aDataSet: TDataSet);
begin
  with aDataSet as IBaseManageDB do
    begin
      TableName := 'DOCUMENTS';
      if Assigned(ManagedFieldDefs) then
        with ManagedFieldDefs do
          begin
            ...
            Add('CHECKSUM',ftString,32,False);
            Add('FULL',ftString,1,False);
            Add('DATE',ftDateTime,0,False);
            Add('CREATEDBY',ftString,4,False);
            Add('CHANGEDBY',ftString,4,False);
            Add('MESSAGE',ftString,100,False);
