...
begin
  ...
  OleCheck(Root.ParseDisplayName(0, nil, PWideChar(WideString(UTF8ToSys(Directory))), chEaten, FolderPidl, dwAttributes));
  try
    OleCheck(Root.BindToObject(FolderPidl, nil, IShellFolder, Pointer(Folder)));
    p := AllocMem(SizeOf(PItemIDList) * 1);
    try
      OleCheck(Folder.ParseDisplayName(0, nil,PWideChar(WideString(UTF8ToSys(Filename))), chEaten, p^[0],dwAttributes));
