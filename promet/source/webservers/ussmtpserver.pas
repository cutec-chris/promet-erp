...
type
  TSSMTPMailEvent = procedure(aSocket : TSTcpThread;aMail : TStrings;aFrom : string;aTo : TStrings) of object;
  TSSMTPAcceptEvent = function(aSocket : TSTcpThread;aFrom : string;aTo : TStrings) : Boolean of object;
...
type
  ...
  published
    ...
    property BannedAttachments: TStringList read FBannedAttachments write FBannedAttachments;
...
resourcestring
  ...
  SSMTPNotAlloweAttachmentFound= 'A not allowed attachment found, mail rejected for security reasons';
...
procedure TSSmtpServer.DoCommandData(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
var
...
// -----------------------------------------------------------------------------
procedure TSSmtpServer.DoCommandError(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
begin
  SendToClient(AThread, '520 '+SSMTPCommandUnknown+' ('+AParam+')');
end;

procedure TSSmtpServer.DoCommandHelo(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
begin
  SendToClient(AThread, '250 '+SSMTPHelo+' '+AThread.Connection.PeerName);
end;

// -----------------------------------------------------------------------------
procedure TSSmtpServer.DoCommandMail(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
var
  LMailFromOK: Boolean;
...
// -----------------------------------------------------------------------------
procedure TSSmtpServer.DoCommandQuit(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
begin
...
// -----------------------------------------------------------------------------
procedure TSSmtpServer.DoCommandRcpt(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
var
  LRcptOK: Boolean;
  i: Integer;
...
// -----------------------------------------------------------------------------
procedure TSSmtpServer.DoCommandRSet(AThread: TSTcpThread; ASession: TSmtpSession; const AParam: string);
begin
  ASession.Clear;
...
// -----------------------------------------------------------------------------
function TSSmtpServer.FindFileType(const AExtension: string; const AMimePart: TMimePart): Boolean;
var
  i: Integer;
begin
  Result:= False;
  ...
  try
    ...
    while ( not AThread.Terminated ) do begin
      ...
      if ( AThread.ReadTimedOut ) then
        ...
        if Assigned(OnLog) then OnLog(AThread,True,LRow);
